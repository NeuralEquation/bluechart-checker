...
function HomePage({ data, go }: { data: DataState; go: (r: Route, s?: string, c?: string) => void }) {
  ...
  const resumeRoute = lastExample ? "check" : RESUMABLE_ROUTES.includes(data.settings.lastRoute as Route)
    ...
    : "examples";
  return <div className="page home-page"><section className="dashboard-hero"><div className="hero-copy"><p className="eyebrow">YOUR STUDY MAP</p><h1>次にやる一問が、<br />すぐ見つかる。</h1><p>進捗はこの端末だけに保存されています。今日の確認を、前回の続きから始めましょう。</p></div><div className="overall-grid"><div className="progress-ring" style={{ "--value": `${overall.percent * 3.6}deg` } as React.CSSProperties}><span><strong>{overall.percent}%</strong><small>確認済み</small></span></div><div className="hero-stats"><div><strong>{overall.total - overall.done}</strong><span>未確認</span></div><div><strong>{overall.review}</strong><span>要復習</span></div><div><strong>{last ? new Date(last).toLocaleDateString("ja-JP", { month: "numeric", day: "numeric" }) : "—"}</strong><span>最終確認</span></div></div></div></section>
    <section className="resume-panel"><div className="resume-icon">↗</div><div className="resume-copy"><p className="eyebrow">CONTINUE</p><h2>{lastExample ? `例題${lastExample.exampleNumber}から再開` : `${SUBJECTS[resumeSubject] ?? "数学"}を開く`}</h2><p>{lastExample ? `${SUBJECTS[lastExample.subjectCode]} · ${chapterLabel(lastExample.chapterCode)} · ${lastExample.description || lastExample.title}` : "前回開いていた科目・章へ戻れます。"}</p></div><button className="resume-button" onClick={() => go(resumeRoute, resumeSubject, resumeChapter)}>前回の続きへ <span>→</span></button></section>
    <section className="quick-actions" aria-label="クイックスタート"><button onClick={() => go("examples", "", "")}><span className="quick-icon">⌕</span><strong>全例題を探す</strong><small>保存済みの条件で再表示</small></button><button onClick={() => go("check", resumeSubject, resumeChapter)}><span className="quick-icon">▶</span><strong>連続チェック</strong><small>苦手な問題から確認</small></button><button onClick={() => go("settings")}><span className="quick-icon">◫</span><strong>対象範囲</strong><small>章・表示を調整</small></button></section>
...
const FILTER_OPTIONS = [
  ["all", "すべて"], ["unseen", "未確認"], ["triangle", "△のみ"],
  ["below2", "○以下"], ["review", "要復習"], ["stale", "30日以上"]
] as const;
...
function ExamplesPage({ data, subjectCode, chapterCode, go, reload, notify, patchSettings }: { data: DataState; subjectCode: string; chapterCode: string; go: (r: Route, s?: string, c?: string) => void; reload: () => Promise<void>; notify: (s: string) => void; patchSettings: (patch: Partial<AppSettings>) => void }) {
  ...
  const update = async (example: StoredCatalogExample, patch: Partial<Progress>, history = false) => {
    ...
    await saveProgress(map.get(example.id), { exampleId: example.id, ...patch }, history); await reload(); notify("保存しました");
  };
  return <div className="page"><button className="back" onClick={() => go(chapterCode ? "chapters" : "home", subjectCode)}>← 戻る</button><div className="list-title"><div><p className="eyebrow">EXAMPLES</p><h1>{chapterCode ? chapterLabel(chapterCode) : subjectCode ? SUBJECTS[subjectCode] : "全例題"}</h1></div><button className="primary compact" onClick={() => go("check", subjectCode, chapterCode)}>連続チェック</button></div>
    <div className="filters"><div className="search-wrap"><span>⌕</span><input type="search" placeholder="例題番号・内容を検索" value={query} onChange={(e) => setQuery(e.target.value)} />{query && <button aria-label="検索をクリア" onClick={() => setQuery("")}>×</button>}</div><div className="filter-chips" aria-label="絞り込み条件">{FILTER_OPTIONS.map(([value, label]) => <button key={value} className={filter === value ? "active" : ""} aria-pressed={filter === value} onClick={() => setFilter(value)}>{label}</button>)}</div><div className="filter-footer"><label>並び順<select value={sort} onChange={(e) => setSort(e.target.value)} aria-label="並び順"><option value="number">例題番号順</option><option value="old">最終確認が古い順</option><option value="low">評価が低い順</option><option value="count">確認回数が少ない順</option></select></label><span><strong>{filtered.length}</strong> 件</span><small>条件は自動保存</small></div></div>
    <div className="example-list">{filtered.slice(0, visible).map((example) => { const p = map.get(example.id); return <article className={`example-card rating-state-${p?.rating ?? 0}`} key={example.id}><div className="example-main"><div className="example-number"><span>EXAMPLE</span>例題{example.exampleNumber}</div><div><h2>{example.description || example.title}</h2><p>{SUBJECTS[example.subjectCode]} · {chapterLabel(example.chapterCode)} · 第{example.sectionNumber}節</p></div></div><RatingButtons value={p?.rating ?? 0} onChange={(rating) => void update(example, { rating }, true)} /><div className="example-tools"><label className="review-toggle"><input type="checkbox" checked={p?.needsReview ?? false} onChange={(e) => void update(example, { needsReview: e.target.checked })} />☆ 要復習</label><span>確認 {p?.reviewCount ?? 0}回</span><span>{p?.lastReviewedAt ? new Date(p.lastReviewedAt).toLocaleDateString("ja-JP") : "未確認"}</span><button className="example-start" onClick={() => { patchSettings({ lastExampleId: example.id }); go("check", example.subjectCode, example.chapterCode); }}>ここから確認 →</button></div><details><summary>動画項目 {example.videoItems.length}件</summary><ul>{example.videoItems.map((v) => <li key={v.contentId}>{v.label}<code>{v.contentId}</code></li>)}</ul></details><label className="memo-label">メモ<MemoField value={p?.memo ?? ""} onSave={(memo) => void update(example, { memo })} placeholder="つまずきや注意点" /></label></article>; })}</div>
...
function CheckPage({ data, subjectCode, chapterCode, go, reload, notify, patchSettings }: { data: DataState; subjectCode: string; chapterCode: string; go: (r: Route, s?: string, c?: string) => void; reload: () => Promise<void>; notify: (s: string) => void; patchSettings: (patch: Partial<AppSettings>) => void }) {
  ...
  return <div className="check-page"><div className="check-top"><button className="back" onClick={() => go("examples", subjectCode, chapterCode)}>× 終了</button><span><strong>{index + 1}</strong> / {queue.length}</span><button className="text-button" disabled={!undo} onClick={() => void undoOnce()}>↶ 取り消す</button></div><div className="check-progress"><i style={{ width: `${((index + 1) / queue.length) * 100}%` }} /></div><article className="check-card"><div className="check-context"><p className="eyebrow">{SUBJECTS[current.subjectCode]} · {chapterLabel(current.chapterCode)}</p><span>{p?.reviewCount ?? 0}回確認</span></div><div className="check-number">例題{current.exampleNumber}</div><h1>{current.description || current.title}</h1><p className="section-note">第{current.sectionNumber}節 · 動画項目 {current.videoItems.length}件</p><div className="large-ratings"><button className={p?.rating === 1 ? "active rating-one" : ""} onClick={() => void apply({ rating: 1 }, true)}><kbd>1</kbd>△<small>解説が必要</small></button><button className={p?.rating === 2 ? "active rating-two" : ""} onClick={() => void apply({ rating: 2 }, true)}><kbd>2</kbd>○<small>自力で解ける</small></button><button className={p?.rating === 3 ? "active rating-three" : ""} onClick={() => void apply({ rating: 3 }, true)}><kbd>3</kbd>◎<small>説明できる</small></button></div><button className={`review-big ${p?.needsReview ? "active" : ""}`} onClick={() => void apply({ needsReview: !p?.needsReview })}>☆ 要復習 {p?.needsReview ? "ON" : "OFF"}</button><label className="memo-label">メモ<MemoField value={p?.memo ?? ""} onSave={(memo) => void apply({ memo })} placeholder="この例題の注意点" /></label></article><div className="check-nav"><button disabled={index === 0} onClick={() => setIndex((x) => x - 1)}>← 前へ</button><button className="next-button" onClick={() => setIndex((x) => Math.min(x + 1, queue.length - 1))}>保留して次へ →</button></div></div>;
}
