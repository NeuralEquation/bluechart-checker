...
export function validateImportFiles(values: unknown[]): ImportResult {
  ...
  values.forEach((value, fileIndex) => {
    if (!isObject(value) || !Array.isArray((value as Partial<ImportFile>).examples)) {
      throw new Error(`繝輔ぃ繧､繝ｫ${fileIndex + 1}: examples驟榊・縺後≠繧翫∪縺帙ｓ縲Ａ);
    }
    for (const raw of (value as unknown as ImportFile).examples) {
      if (!isObject(raw) || typeof raw.id !== "string" || typeof raw.exampleNumber !== "number" ||
        ...
        throw new Error(`繝輔ぃ繧､繝ｫ${fileIndex + 1}: 蠢・磯・岼縺御ｸ肴ｭ｣縺ｪ萓矩｡後′縺ゅｊ縺ｾ縺吶Ａ);
      }
      ...
      seenIds.add(raw.id);
      if (!/^\d{8}$/.test(raw.id)) warnings.push({ type: "id", message: `${raw.id}: 萓矩｡栗D縺・譯√〒縺ｯ縺ゅｊ縺ｾ縺帙ｓ縲Ａ });
      ...
      const cleanVideos = raw.videoItems.filter((video) => {
        ...
        if (!/^\d{10}$/.test(video.contentId)) warnings.push({ type: "contentId", message: `${video.contentId}: 蜍慕判ID縺・0譯√〒縺ｯ縺ゅｊ縺ｾ縺帙ｓ縲Ａ });
        return true;
      });
      if (typeof raw.videoCount === "number" && raw.videoCount !== raw.videoItems.length) {
        warnings.push({ type: "videoCount", message: `${raw.id}: videoCount縺ｨ驟榊・莉ｶ謨ｰ縺御ｸ閾ｴ縺励∪縺帙ｓ縲Ａ });
      }
  ...
  for (const subjectCode of Object.keys(subjects)) {
    ...
    if (missing.length) warnings.push({ type: "missing", message: `遘醍岼繧ｳ繝ｼ繝・{subjectCode}: 谺逡ｪ ${missing.join(", ")}` });
  }
  if (duplicateIds.length) warnings.push({ type: "duplicate", message: `驥崎､・ｾ矩｡栗D ${duplicateIds.length}莉ｶ` });
  if (duplicateContentIds.length) warnings.push({ type: "duplicateContent", message: `驥崎､・虚逕ｻID ${duplicateContentIds.length}莉ｶ` });
...
export async function readJsonFiles(files: FileList | File[]): Promise<unknown[]> {
  ...
  if (!list.length) throw new Error("JSON繝輔ぃ繧､繝ｫ繧帝∈謚槭＠縺ｦ縺上□縺輔＞縲・);
  for (const file of list) if (!file.name.toLowerCase().endsWith(".json")) throw new Error(`${file.name}: JSON繝輔ぃ繧､繝ｫ縺ｧ縺ｯ縺ゅｊ縺ｾ縺帙ｓ縲Ａ);
  return Promise.all(list.map(async (file) => {
    try { return JSON.parse(await file.text()); }
    catch { throw new Error(`${file.name}: JSON縺ｨ縺励※隱ｭ縺ｿ蜿悶ｌ縺ｾ縺帙ｓ縲Ａ); }
  }));
}
